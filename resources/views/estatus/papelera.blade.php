...
@section('page-title', 'Papelera de Estatus')
@section('page-description', 'Elementos eliminados')

@section('content')
<div class="mb-6 flex justify-between items-center">
    <div>
        <h3 class="text-lg font-semibold text-slate-800">Papelera de Estatus</h3>
        <p class="text-sm text-slate-500 mt-1">Elementos eliminados que pueden ser restaurados</p>
    </div>
    <a href="{{ route('estatus.index') }}" class="bg-slate-200 text-slate-700 px-4 py-2 rounded-lg hover:bg-slate-300 transition flex items-center gap-2">
...
<div class="bg-white rounded-lg shadow overflow-hidden">
    <table class="min-w-full divide-y divide-slate-200">
        ...
        <tbody class="bg-white divide-y divide-slate-200">
            @forelse($estatuss as $estatus)
                <tr class="hover:bg-slate-50">
                    <td class="px-6 py-4 text-sm text-slate-900">{{ $estatus->entidad }}</td>
                    <td class="px-6 py-4 text-sm text-slate-900">{{ $estatus->nombre }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                        {{ $estatus->deleted_at->format('d/m/Y H:i') }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <form action="{{ route('estatus.restore', $estatus->id) }}" method="POST" class="inline">
                            @csrf
                            <button type="submit" class="text-green-600 hover:text-green-900 mr-3">Restaurar</button>
                        </form>
                        <form action="{{ route('estatus.forceDelete', $estatus->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
