...
@section('page-title', 'Crear Estatus')
@section('page-description', 'Registrar un nuevo estado')
        ...
        <form action="{{ route('estatus.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="entidad" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                    ...
                    <option value="curso" {{ old('entidad') == 'curso' ? 'selected' : '' }}>Curso</option>
                    <option value="cohorte" {{ old('entidad') == 'cohorte' ? 'selected' : '' }}>Cohorte</option>
                    <option value="inscripcion" {{ old('entidad') == 'inscripcion' ? 'selected' : '' }}>Inscripción</option>
                    <option value="certificado" {{ old('entidad') == 'certificado' ? 'selected' : '' }}>Certificado</option>
                </select>
                @error('entidad')
            ...
            <div class="mb-4">
                ...
                <input type="number" name="orden_visual" value="{{ old('orden_visual', 0) }}" 
                    class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" 
                    min="0">
                @error('orden_visual')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
