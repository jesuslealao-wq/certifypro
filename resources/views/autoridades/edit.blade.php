...
@section('page-description', 'Modificar información de la autoridad')

@section('content')
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('autoridades.update', $autoridad) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            
            <div class="mb-4">
                <label class="block text-sm font-medium text-slate-700 mb-2">Nombre Completo</label>
                <input type="text" name="nombre_completo" value="{{ old('nombre_completo', $autoridad->nombre_completo) }}" 
            ...
            <div class="mb-4">
                ...
                <input type="text" name="cargo" value="{{ old('cargo', $autoridad->cargo) }}" 
                    class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" 
                    required>
                @error('cargo')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="especialidad" value="{{ old('especialidad', $autoridad->especialidad) }}" 
                    class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                @error('especialidad')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    @if($autoridad->sello_path)
                        <div class="mb-3">
                            <p class="text-xs text-slate-600 mb-2">Sello actual:</p>
                            <img src="{{ asset('storage/' . $autoridad->sello_path) }}" alt="Sello" class="h-32 border border-slate-300 rounded-lg">
                        </div>
                    @endif
                    ...
                    <input type="file" name="sello" id="sello-input" accept=".jpg,.jpeg" 
                        ...
                        class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    <p class="text-xs text-slate-500 mt-1">{{ $autoridad->sello_path ? 'Subir nuevo sello (opcional)' : 'Formato: JPG, Máx: 2MB' }}</p>
                    @error('sello')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                    
                    <div id="sello-editor" class="mt-3 hidden">
                        <p class="text-xs text-slate-600 mb-2">Nuevo sello:</p>
                ...
                <div>
                    ...
                    @if($autoridad->firma_path)
                        <div class="mb-3">
                            <p class="text-xs text-slate-600 mb-2">Firma actual:</p>
                            <img src="{{ asset('storage/' . $autoridad->firma_path) }}" alt="Firma" class="h-32 border border-slate-300 rounded-lg">
                        </div>
                    @endif
                    ...
                    <input type="file" name="firma" id="firma-input" accept=".jpg,.jpeg" 
                        ...
                        class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    <p class="text-xs text-slate-500 mt-1">{{ $autoridad->firma_path ? 'Subir nueva firma (opcional)' : 'Formato: JPG, Máx: 2MB' }}</p>
                    @error('firma')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                    
                    <div id="firma-editor" class="mt-3 hidden">
                        <p class="text-xs text-slate-600 mb-2">Nueva firma:</p>
                        <div class="border border-slate-300 rounded-lg p-4 bg-slate-50">
                            <div class="flex justify-center items-center bg-white border border-slate-200 rounded" style="height: 200px; overflow: hidden;">
            ...
            <div class="mb-4">
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="activo" value="1" {{ old('activo', $autoridad->activo) ? 'checked' : '' }} 
