...
@section('page-title', 'Emitir Certificado')
@section('page-description', 'Crear un nuevo certificado')
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('certificados.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="codigo_registro_manual" value="{{ old('codigo_registro_manual') }}" 
                    class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    <div>
                        ...
                        <select name="firma_1_id" class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm">
                            ...
                            @foreach($autoridades as $autoridad)
                                <option value="{{ $autoridad->id }}">{{ $autoridad->nombre_completo }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
            ...
            <div class="mb-4">
                ...
                <select name="plantilla_pdf_id" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    @foreach($plantillas as $plantilla)
                        <option value="{{ $plantilla->id }}" {{ old('plantilla_pdf_id') == $plantilla->id ? 'selected' : '' }}>
