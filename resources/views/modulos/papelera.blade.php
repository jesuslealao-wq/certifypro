...
@section('page-title', 'Papelera de Módulos')
@section('page-description', 'Elementos eliminados')

@section('content')
<div class="mb-6 flex justify-between items-center">
    <div>
        <h3 class="text-lg font-semibold text-slate-800">Papelera de Módulos</h3>
        <p class="text-sm text-slate-500 mt-1">Elementos eliminados que pueden ser restaurados</p>
    </div>
    <a href="{{ route('modulos.index') }}" class="bg-slate-200 text-slate-700 px-4 py-2 rounded-lg hover:bg-slate-300 transition flex items-center gap-2">
            ...
            @forelse($modulos as $modulo)
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-6 py-4 text-sm text-slate-900">
                        @if($modulo->curso)
                            {{ $modulo->curso->nombre_curso }}
                        @else
                            <span class="text-slate-400">Curso eliminado</span>
                        @endif
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                        {{ $modulo->deleted_at->format('d/m/Y H:i') }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <form action="{{ route('modulos.restore', $modulo->id) }}" method="POST" class="inline">
                            @csrf
                            <button type="submit" class="text-green-600 hover:text-green-900 mr-3">Restaurar</button>
                        </form>
                        <form action="{{ route('modulos.forceDelete', $modulo->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
