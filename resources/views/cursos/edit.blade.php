...
@section('page-description', 'Modificar información del curso')
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('cursos.update', $curso) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nombre_curso" value="{{ old('nombre_curso', $curso->nombre_curso) }}" 
            ...
            <div class="mb-4">
                ...
                <input type="number" name="horas_academicas" value="{{ old('horas_academicas', $curso->horas_academicas) }}" 
                    class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" 
                    required min="1">
            ...
            <div class="mb-4">
                ...
                <select name="estado_id" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    @foreach($estados as $estado)
                        <option value="{{ $estado->id }}" {{ old('estado_id', $curso->estado_id) == $estado->id ? 'selected' : '' }}>
                            {{ $estado->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <textarea name="descripcion" rows="4" 
                    class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('descripcion', $curso->descripcion) }}</textarea>
                @error('descripcion')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
